...
<div class="mx-auto max-w-2xl py-8 animate-fade-in">
    ...
    <p class="text-zinc-500 dark:text-zinc-400 text-sm mb-8">{{ __('Please read these terms carefully before using noref.to.') }}</p>

    <div class="space-y-6">
        {{-- Acceptance of Terms --}}
        <section>
            <h2 class="font-heading font-bold text-lg text-zinc-900 dark:text-zinc-100 mb-2">{{ __('Acceptance of Terms') }}</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{{ __('By accessing or using noref.to, you agree to be bound by these Terms of Service. If you do not agree to these terms, please do not use the service.') }}</p>
        </section>

        {{-- Description of Service --}}
        <section>
            <h2 class="font-heading font-bold text-lg text-zinc-900 dark:text-zinc-100 mb-2">{{ __('Description of Service') }}</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{{ __('noref.to provides a free, privacy-first URL shortening and anonymous redirect service. We allow users to create shortened links and redirect through URLs without exposing referrer information.') }}</p>
        </section>

        {{-- Acceptable Use --}}
        <section>
            <h2 class="font-heading font-bold text-lg text-zinc-900 dark:text-zinc-100 mb-2">{{ __('Acceptable Use') }}</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{{ __('You may use noref.to for lawful purposes only. You are responsible for all content you link to through our service.') }}</p>
        </section>

        {{-- Prohibited Content --}}
        <section>
            <h2 class="font-heading font-bold text-lg text-zinc-900 dark:text-zinc-100 mb-2">{{ __('Prohibited Content') }}</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed mb-2">{{ __('You may not use noref.to to link to content that:') }}</p>
        ...
        <section>
            ...
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{{ __('noref.to is provided "as is" without any warranties. We do not guarantee uptime, availability, or the permanence of any shortened link. We are not liable for any damages arising from your use of this service.') }}</p>
        </section>
