...
<div class="mx-auto max-w-2xl py-8 animate-fade-in">
    ...
    <p class="text-zinc-500 dark:text-zinc-400 text-sm mb-8">{{ __('Learn more about noref.to and why it exists.') }}</p>

    <div class="space-y-6">
        {{-- What is noref.to? --}}
        <section>
            <h2 class="font-heading font-bold text-lg text-zinc-900 dark:text-zinc-100 mb-2">{{ __('What is noref.to?') }}</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{{ __('noref.to is a free, privacy-first URL shortener and anonymous redirect service. It lets you create short links or redirect through any URL without exposing your identity or browsing habits to the destination site.') }}</p>
        </section>

        {{-- Why noref.to Exists --}}
        <section>
            <h2 class="font-heading font-bold text-lg text-zinc-900 dark:text-zinc-100 mb-2">{{ __('Why noref.to Exists') }}</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{{ __('Online privacy is eroding. Every link you click can reveal where you came from, what you were reading, and who you are. noref.to breaks that tracking chain. When you redirect through us, the destination sees our server — not you.') }}</p>
        </section>

        {{-- How It Works --}}
        <section>
            <h2 class="font-heading font-bold text-lg text-zinc-900 dark:text-zinc-100 mb-2">{{ __('How It Works') }}</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed mb-2">{{ __('noref.to offers two modes:') }}</p>
            <ul class="list-disc list-inside text-sm text-zinc-600 dark:text-zinc-400 space-y-1">
                ...
                <li>{{ __('Direct Redirect mode — use the format noref.to/?url to redirect instantly. Nothing is stored, nothing is logged.') }}</li>
            </ul>
        </section>
